...
**触发条件**（满足以下任一即进入待观察）：
1. **隐性修正**：用户未明说"以后都这样"，但修改了 Claude 输出中的某类模式（如调整表述风格、删除某类措辞、重排结构）
2. **单次偏好声明**：用户说"这里用 X 而不是 Y"，但未明确表示该偏好适用于所有项目
3. **工具/参数选择**：用户在特定分析中选择了非默认工具或参数配置（如指定 CiteSpace 而非 VOSviewer）
4. **格式调整**：用户要求修改图表标注方式、引用格式、或章节排列顺序

**"同类反馈"判定标准**：
两次反馈属于"同类"需同时满足：
- **同一维度**：涉及相同的偏好分类（写作风格 / 章节结构 / 图表风格 / 工具技术 / 方法论 / 期刊规范 / 工作流）
- **同一方向**：两次修改指向相同倾向（如两次都要求精简措辞，而非一次精简、一次扩展）
- 两次反馈可来自同一项目的不同阶段，也可来自不同项目

**升级机制**：
- 待观察条目出现第 2 次 → 向用户确认后升级为对应分类的正式规则，从待观察表中移除
- 超过 3 个月未再出现 → 在待观察表中标注"已过期"，不自动删除（供回溯参考）

**待观察记录格式**：
```
| 日期 | 建议内容 | 出现次数 | 状态 |
|------|---------|---------|------|
| YYYY-MM-DD | [简述反馈内容及来源场景] | 1 | 观察中 |
```
...
**待观察记录时告知用户**：
```
注意到你[具体修改行为]。已记入待观察（第1次出现）。
若后续再次出现相同偏好，将升级为正式规则。
若这纯属本次特殊需求，可忽略。
```
